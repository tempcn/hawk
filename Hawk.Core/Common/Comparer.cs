...
namespace Hawk.Common
{
    public class Comparer
    {
        ...
        public static void Sort(int[] s, bool isAsc = false)
        {
            if (!isAsc)
                Array.Sort(s, (x, y) => x - y);
            else
                Array.Sort(s, delegate (int x, int y) { return y - x; });
            //if (orderBy)
            //    Array.Sort<int>(s, new IntComparer(orderBy));
            //else
            //    Array.Sort<int>(s);
        }
